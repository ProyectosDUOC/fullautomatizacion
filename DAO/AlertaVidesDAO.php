...
require_once($rootDir . "/Modelo/AlertaVides.php");

class AlertaVidesDAO {
    
    public static function ultimoId(){
        $cc = DB::getInstancia();
        $stSql = "SELECT * FROM alerta_vides order by id_alerta desc  limit 1";
        $rs = $cc->db->prepare($stSql);
        $rs->execute();
        $ba = $rs->fetch();
        $nuevo = new AlertaVides($ba['id_alerta'],
                            $ba['asunto'],
                            $ba['comentario'],
                            $ba['fecha'], 
                            $ba['activo']);
    ...
    public static function agregar($nuevo) {
        $cc=DB::getInstancia();
        $stSql = "INSERT INTO alerta_vides VALUES ";
    ...
    public static function readAll() { 
        $cc = DB::getInstancia();
        $stSql = "SELECT * FROM alerta_vides order by id_alerta asc";
        $rs = $cc->db->prepare($stSql);
        $rs->execute();
        $ba = $rs->fetchAll();
        ...
        $pila = array();
        foreach ($ba as $c) {
            $nuevo = new AlertaVides($c['id_alerta'],
                                $c['asunto'],
                                $c['comentario'],
                                $c['fecha'],
                                $c['activo']);
